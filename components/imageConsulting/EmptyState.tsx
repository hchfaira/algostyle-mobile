 ...
 * Quiet luxury — nude palette, black text, animated orbs
 */
...
function PulseOrb({ color, icon, delay }: { color: string; icon: string; delay: number }) {
  ...
  return (
    <Animated.View
      ...
      style={[styles.orb, { backgroundColor: color }, style, Shadow.md]}
    >
    ...
    >
      ...
      <Animated.View entering={FadeInDown.delay(80).duration(400)} style={styles.orbGrid}>
        {PREVIEW_ORBS.map(({ color, icon }, i) => (
          <PulseOrb key={i} color={color} icon={icon} delay={i * 80} />
        ))}
      </Animated.View>
      ...
      <Animated.View entering={FadeInDown.delay(320).duration(350)} style={styles.trustRow}>
        {TRUST_ITEMS.map(({ icon, label }) => (
          <View key={label} style={styles.trustItem}>
            <Ionicons name={icon as any} size={13} color={GRAY} />
...
const styles = StyleSheet.create({
  ...
  trustLabel: { fontSize: FontSize.xs, color: GRAY, fontWeight: FontWeight.medium },
